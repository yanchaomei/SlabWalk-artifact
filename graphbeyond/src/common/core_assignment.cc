...
template <>
void CoreAssignment<interleaved>::set_core_sequence() {
  ...
  std::iota(node1_cores.begin(), node1_cores.begin() + physical_cores_per_socket_, 1 * physical_cores_per_socket_);
  std::iota(node0_cores.begin(), node0_cores.begin() + physical_cores_per_socket_, 0 * physical_cores_per_socket_);

  // with hyper-threading
  if (hyperthreading_enabled()) {
    std::iota(node1_cores.begin() + physical_cores_per_socket_, node1_cores.end(), 3 * physical_cores_per_socket_);
    std::iota(node0_cores.begin() + physical_cores_per_socket_, node0_cores.end(), 2 * physical_cores_per_socket_);
  }
...
template void CoreAssignment<interleaved>::print_hardware_info() const;
