...
public:
  ...
  void connect(Context& channel_context, u16 lid, QP& other_qp) const {
    ...
    send_buffer.lid = lid;
    send_buffer.qp_number = qp->get_qp_num();
    // RoCE: populate gid + gid_index so the remote side can build a global AH
    send_buffer.gid = channel_context.get_gid();
    send_buffer.gid_index = channel_context.get_gid_index();
