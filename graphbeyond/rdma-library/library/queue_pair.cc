...
void QueuePair::transition_to_rtr(const QPInfo& remote_buffer) {
  ...
  // CL fabric note: ibv_rc_pingpong succeeded with -m 1024; jumbo (4096) may
  // not be supported cleanly by the CloudLab RoCEv2 fabric. Use 1024 to match.
  attributes.path_mtu = IBV_MTU_1024;
  attributes.dest_qp_num = remote_buffer.qp_number;
  attributes.rq_psn = 0;
  attributes.max_dest_rd_atomic = 16;
  // Bumped from 12 to 0x14 (20) for more tolerant RNR behavior on CL fabric.
  attributes.min_rnr_timer = 0x14;
  attributes.ah_attr.sl = 0;
  attributes.ah_attr.src_path_bits = 0;
  attributes.ah_attr.port_num = context_->get_config().device_port;

  // RoCE: use global routing (GID-based) when on Ethernet link layer.
  if (context_->is_roce()) {
    attributes.ah_attr.is_global = 1;
    attributes.ah_attr.grh.dgid = remote_buffer.gid;
    attributes.ah_attr.grh.sgid_index = context_->get_gid_index();
...
void QueuePair::transition_to_rts() {
  ...
  // Bumped timeout from 14 (~67ms) to 18 (~17s) for CL fabric tolerance.
  // retry_cnt=7 and rnr_retry=7 are already at max.
  attributes.timeout = 18;
