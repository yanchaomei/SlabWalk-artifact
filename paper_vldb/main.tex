...
while all CNs can compute a Slab owner from the descriptor.  A separate
fixed-pool diagnostic across SIFT, DEEP, and GIST preserved recall but failed
its preregistered three-CN scaling gate on all three datasets and its fairness
gate on SIFT and GIST.  We retain that complete negative campaign in the
artifact and make no aggregate multi-CN throughput claim.  Online maintenance
and coordinated version admission remain separate database responsibilities.
