...
\begin{itemize}
  ...
  state, MN storage, transfer, and representation error.  Construction derives
  Slabs and resident navigation from one validated snapshot, and a verified
  descriptor records the format and placement.  Rematerialization is explicitly
  offline.
\end{itemize}
...
\end{align}
These are deployment-feasibility constraints, not an optimizer.  An operator
measures a small candidate sweep, rejects points that exceed a resource or
recall target, and records the selected layout metadata.  Section~\ref{sec:eval-index-cost}
illustrates the procedure with the measured GIST budget sweep.
...
design require?  \textbf{Q4:} What are the cost and supported semantics of
...
no-batching traces.  Construction uses five independent derived-build runs with
...
infeasible points, then choose the highest-throughput retained policy.  This is
the worked use of Eq.~\ref{eq:deployment-constraints}; SlabWalk does not predict
unmeasured points or claim an online optimizer.
...
Eq.~\ref{eq:deployment-constraints} are discarded.  The GIST budget sweep in
Figure~\ref{fig:eval-index-cost}(a,b) is one measured instance of this process:
